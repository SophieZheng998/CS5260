...
from openke.module.model import TransD, TransE, TransR, ComplEx, DistMult, TransH
...
class KGAT(nn.Module):
    ...
    def __init__(self, args,
        ...
        if self.kg_embedding_model_name == "transe":
            self.kg_embedding_model = TransE()
        # openke TransR
        elif self.kg_embedding_model_name == "transr":
            self.kg_embedding_model = TransR()
        # openke TransD
        ...
        # openke ComplEx
            ...
            self.kg_embedding_model = ComplEx()
        # openke DistMult
        elif self.kg_embedding_model_name == "distmult":
            self.kg_embedding_model = DistMult()
        # openke TransH
        # TODO
        elif self.kg_embedding_model_name == "transh":
            self.kg_embedding_model = TransH(
                ent_tot = n_entities + self.n_users,
	            rel_tot = n_relations,
	            dim = self.embed_dim, 
	            p_norm = 1, 
    ...
    # openke TransH KG Loss
    # TODO
    def calc_kg_loss_transh(self, h, r, pos_t, neg_t):
        ...
        r_embed = self.kg_embedding_model.rel_embeddings(r)                                                                                                        
        h_embed = self.kg_embedding_model.ent_embeddings(h)                                            
        pos_t_embed = self.kg_embedding_model.ent_embeddings(pos_t)                                    
        neg_t_embed = self.kg_embedding_model.ent_embeddings(neg_t)  
        r_norm = self.kg_embedding_model.norm_vector(r)                      

        l2_loss = (torch.mean(h_embed ** 2) + 
    ...
    def update_attention_batch_transd(self, h_list, t_list, r_idx):
        ...
        # openke TransH attention batch
    def update_attention_batch_transh(self, h_list, t_list, r_idx):
    ...
    def update_attention(self, h_list, t_list, r_list, relations):
        ...
        for r_idx in relations:
            ...
            # TODO
                ...
                batch_v_list = self.update_attention_batch_transh(batch_h_list, batch_t_list, r_idx)
            # openke ComplEx
            elif self.kg_embedding_model_name == "complex":
    ...
    def forward(self, *input, mode):
        ...
        if mode == 'train_kg':
            ...
            # openke TransH
                ...
                return self.calc_kg_loss_transh(*input)
            # openke ComplEx
            elif self.kg_embedding_model_name == "complex":
